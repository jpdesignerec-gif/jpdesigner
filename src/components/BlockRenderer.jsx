...
function RenderBlock({block}){const d=block.data||{};const common={id:block.anchor||undefined,className:`content-block block-${block.type} ${block.className||''}`};
  ...
  if(block.type==='mediaText')return <section {...common}><div className={`block-inner media-text image-${d.imageSide||'left'}`}><button className={`media-frame ${ratioClass(d.ratio)}`} onClick={()=>d.image&&window.open(d.image,'_blank')}><img loading="lazy" decoding="async" src={d.image} alt={d.alt||''} style={{objectFit:d.fit||'cover'}}/></button><div><span className="eyebrow">{d.eyebrow}</span><h2>{d.title}</h2><p>{d.text}</p>{d.buttonLabel&&<a className="button primary" href={d.buttonUrl}>{d.buttonLabel}<ExternalLink/></a>}</div></div></section>;
  if(block.type==='image')return <section {...common}><figure className={`block-image size-${d.size||'large'} radius-${d.radius||'medium'} ${ratioClass(d.ratio)}`}><img loading="lazy" decoding="async" src={d.src} alt={d.alt||''} style={{objectFit:d.fit||'cover'}} onClick={()=>d.expandable&&window.open(d.src,'_blank')}/>{d.caption&&<figcaption>{d.caption}</figcaption>}</figure></section>;
  if(block.type==='banner'||block.type==='fullscreen')return <section {...common}><div className={`${block.type}-media height-${d.height||'medium'} ${d.overlay?'has-overlay':''}`}><img loading="lazy" decoding="async" src={d.src} alt={d.alt||''} style={{objectPosition:d.position||'center'}}/>{d.title&&<div className={`banner-copy align-${d.align||'left'}`}><h2>{d.title}</h2></div>}</div></section>;
  if(block.type==='gallery')return <section {...common}><div className={`block-gallery layout-${d.layout||'mosaic'} cols-${d.columns||3}`}>{(d.images||[]).map((src,i)=><a href={src} target="_blank" rel="noreferrer" className={ratioClass(d.ratio)} key={`${src}-${i}`}><img loading="lazy" decoding="async" src={src} alt={`Galería ${i+1}`} style={{objectFit:d.fit||'cover'}}/></a>)}</div></section>;
  ...
  if(block.type==='button')return <section {...common}><div className={`block-inner block-action align-${d.align||'left'}`}><a className={`button ${d.style||'primary'}`} href={d.url} target={d.newTab?'_blank':undefined} rel={d.newTab?'noreferrer':undefined}>{d.label}{d.icon==='arrow'&&<ExternalLink/>}</a></div></section>;
  ...
  if(block.type==='logo')return <section {...common}><div className={`block-inner logo-strip layout-${d.layout||'row'} ${d.monochrome?'monochrome':''}`}>{(d.logos||[]).map((logo,i)=><img loading="lazy" decoding="async" src={logo.src} alt={logo.alt||`Logo ${i+1}`} key={i}/>)}</div></section>;
